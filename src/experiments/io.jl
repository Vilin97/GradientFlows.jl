...
function experiment_filename(experiment::GradFlowExperiment; kwargs...)
    d, n = size(experiment.problem.u0)
    return experiment_filename(experiment.problem.name, "$(experiment.problem.solver)", d, n, experiment.num_solutions; kwargs...)
end
...
function best_model(problem_name, d; path=joinpath("data", "models"))
    dir = dirname(model_filename(problem_name, d, 1; path=path))
    filenames = readdir(dir)
    if isempty(filenames)
        return nothing
    end
    get_n(f) = parse(Int, f[3])
