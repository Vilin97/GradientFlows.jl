...
function diffusion_problem(d, n, solver_; t0::F=1.0, t_end::F=2.0, dt::F=0.01, rng=DEFAULT_RNG) where {F}
    if solver_ isa SBTM && F == Float64
        return diffusion_problem(d, n, solver_; t0=Float32(t0), t_end=Float32(t_end), dt=Float32(dt), rng=rng)
    end
